...
common_prog(int nargs, char **args)
{
...
#endif
	//kprintf("system answer the call, thread forking...\n");
	result = thread_fork(args[0] /* thread name */,
			args /* thread arg */, nargs /* thread arg */,
			cmd_progthread, NULL);
	//kprintf("thread_forked\n");
	if (result) {
	...
	//kprintf("menu wait pid %d\n", child);
	waitpid(child, &child, 0, &result);
	if (result) {
		kprintf("wait in menu failed: %s\n", strerror(result));
		return result;
	}
...
cmd_df(int nargs, char **args)
{
	...
	char* flag_num = args[1];
	char* status = args[2];
	
	int flag_length=strlen(flag_num);
	//kprintf("%d\n", flag_length);
	if (flag_length > 2 || flag_length == 0){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	if (flag_length == 1 && (flag_num[0]>'9' || flag_num[0]<'1')){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}
	
	if (flag_length == 2 && (flag_num[0]!='1' || flag_num[1]>'2' || flag_num[1]<'0')){
		kprintf("Usage: df nr on/off\n");
		return EINVAL;
	}

	int flag = atoi(flag_num);
	flag --;

	if (strcmp(status, "on") == 0){
		int force_on = 1<<flag;
		dbflags = dbflags | force_on;
	}
	
	else if (strcmp(status, "off") == 0){
		int force_off = ~(1<<flag);
		dbflags = dbflags & force_off;
	}
